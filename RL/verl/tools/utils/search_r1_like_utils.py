...
_cache_file = "search_cache.db"

def _init_cache_db():
    """Initialize the cache database only once"""
        
    with sqlite3.connect(_cache_file) as conn:
        cursor = conn.cursor()
        cursor.execute(
            """
...
def perform_single_search_batch(
    ...
    search_dicts = _get_cached_results(query)
    if search_dicts is not None:
        final_result = _passages2string(search_dicts)
        print("hit cache.")
        ...
        logger.info(f"Batch search: Successful, got 1 total results")
    ...
    if error_msg:
        ...
        try:
            raw_results = [api_response["data"]["originalOutput"]["organic"]] # 只有一个query
            _cache_results(query, raw_results[0])
            if raw_results:
                pretty_results = []
                total_results = 0
            ...
            result_text = json.dumps({"result": "Failed to process search results, try again."})
...
# perform_single_search_batch("https://idealab.alibaba-inc.com/api/v1/search/search", ["hello world"])
